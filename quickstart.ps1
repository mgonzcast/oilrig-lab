...
switch ($choice) {
    "1" {
        Write-Host "`nBuilding base box with Packer..." -ForegroundColor Green
        Set-Location packer
        packer init .
    ...
    "3" {
        ...
        Write-Host "2. waterfalls (Exchange 2019)"
        Write-Host "3. endofroads (SQL Server 2019)"
        
        $vmChoice = Read-Host "`nEnter VM number (1-3)"
