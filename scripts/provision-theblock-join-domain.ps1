...
if ($adapter) {
    ...
    if ($currentIP){
        ...
        if ($currentIP -ne $ClientIPAddress) {
          ...
          New-NetIPAddress -InterfaceIndex $adapter.ifIndex -IPAddress $ClientIPAddress -PrefixLength 24 -ErrorAction SilentlyContinue
          Set-DnsClientServerAddress -InterfaceIndex $adapter.ifIndex -ServerAddresses $DCAddress
          #Write-Host "Network configured: $ClientIPAddress" -ForegroundColor Green
        }
    }
}
