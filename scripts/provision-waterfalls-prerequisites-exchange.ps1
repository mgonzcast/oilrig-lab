...
# REVERT DNS TO DHCP so Windows Features can be installed
Write-Host "Reverting DNS on '$($adapterVagrant.Name)' to DHCP for internet access..." -ForegroundColor Cyan
try {
    # This resets the DNS server addresses to be obtained via DHCP
    Set-DnsClientServerAddress -InterfaceAlias $($adapterVagrant.Name) -ResetServerAddresses -ErrorAction Stop
